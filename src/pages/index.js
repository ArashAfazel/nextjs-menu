    ...
    <>
      <div className="w-full  flex justify-between items-center px-2.5">
        <div className="flex items-center ">
          ...
          <p
            className={`flex items-center ${lobster.className} text-[20px] text-cream`}
          >
            BrandLogo
          </p>
        </div>
      ...
      <div
        className={`w-full  flex flex-col items-center  gap-y-5 ${lobster.className}`}
      >
        {categories.map((item) => (
          <CategoryCard item={item} key={categories.id} />
        ))}
      </div>
    </>
  );
